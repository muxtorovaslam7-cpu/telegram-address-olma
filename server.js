...
app.post(`/webhook/:secret`, async (req, res) => {
  ...
  // 1) Lokatsiya keldi (haqiqiy Telegram location turi) -> kutish holatiga qo'yamiz
  if (message.location) {
  ...
  // 1b) Ba'zi Telegram mijozlari (Web/Desktop) lokatsiyani havola
  // ko'rinishida yuboradi, masalan:
  // https://maps.google.com/maps?q=39.63,66.92&ll=39.63,66.92&z=16
  // Shu havoladan koordinatani ajratib olamiz.
  if (message.text) {
    const coordMatch = message.text.match(/[?&]q=(-?\d+\.\d+),(-?\d+\.\d+)/);
    if (coordMatch) {
